...
namespace Barebones.MasterServer
{
    ...
    public class LobbyPropController : MonoBehaviour
    {
        ...
        public virtual void OnValueChanged()
        {
            ...
            {
                {RawData.PropertyKey, currentValue }
            };

            var loadingPromise = BmEvents.Channel.FireWithPromise(BmEvents.Loading, "Updating");

            LobbiesModule.SetLobbyProperties(changes, error =>
            {
                loadingPromise.Finish();

                if (error != null)
                {
                    DialogBoxView.ShowError("Failed to update the value");
                    RestoreLastValue();
                }
            });
        }
